...
PlasmoidItem {
    ...
    // Get the index based on the device name
    function captureDeviceIndexFromName() {
        // List of the devices
        const devs = audioEngine.getCaptureDevices() || []

        const name = plasmoid.configuration.captureDeviceName || ""
        const idx = devs.indexOf(name)
        return (idx >= 0) ? idx : 0
    }
    ...
    AudioEngine {
        ...
        Component.onCompleted: {
            ...
            setFalloff(0.5)
            setBarHeight(0.1)
            startCapture(captureDeviceIndexFromName())
        }
    }

    // Start capture when the widget loads
    Component.onCompleted: {
        audioEngine.startCapture(captureDeviceIndexFromName())
    }
